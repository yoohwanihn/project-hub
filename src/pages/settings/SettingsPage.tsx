  ...
  return (
    <button
      ...
      className={cn(
        ...
        value ? 'bg-zinc-800 dark:bg-white' : 'bg-zinc-200 dark:bg-zinc-600',
      )}
    >
      <span className={cn(
        'absolute top-[3px] w-4 h-4 bg-white dark:bg-zinc-900 rounded-full shadow-sm transition-transform',
...
function ProfileSection() {
  ...
  return (
    <div className="space-y-6">
      {/* 프로필 사진 */}
      <div>
        <h3 className="text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wide mb-3">프로필 사진</h3>
        <div className="flex items-center gap-5">
          ...
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-zinc-400 dark:text-zinc-500">JPG, PNG, WebP · 최대 10MB</p>
          </div>
        </div>
      </div>
      ...
      {/* 기본 정보 */}
      <div>
        <h3 className="text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wide mb-3">기본 정보</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-semibold text-zinc-600 dark:text-zinc-300 mb-1.5">이름 *</label>
            <input
              className="input"
              value={name}
          ...
          <div>
            <label className="block text-xs font-semibold text-zinc-600 dark:text-zinc-300 mb-1.5">이메일 *</label>
            <input
              type="email"
              className="input"
              value={email}
...
function SecuritySection() {
  ...
  function PwInput({
    ...
    return (
      <div>
        <label className="block text-xs font-semibold text-zinc-600 dark:text-zinc-300 mb-1.5">{label}</label>
        <div className="relative">
          <input
  ...
  return (
    <div className="space-y-5 max-w-sm">
      <h3 className="text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">비밀번호 변경</h3>
      ...
      />
      <div>
        <label className="block text-xs font-semibold text-zinc-600 dark:text-zinc-300 mb-1.5">새 비밀번호 확인</label>
        <input
          type="password"
...
function NotificationsSection() {
  ...
  function Row({
    ...
    return (
      <div className={cn(
        'flex items-center justify-between p-3.5 rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-700',
        indent && 'ml-4',
      )}>
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between p-3.5 rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-700">
        <div className="min-w-0 mr-3">
          <p className="text-sm font-medium text-zinc-700 dark:text-zinc-200">브라우저 푸시 알림</p>
          ...
          {settings.push_enabled && !pushGranted && (
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">브라우저 권한이 필요합니다. 주소창 자물쇠 아이콘에서 허용해 주세요.</p>
          )}
        </div>
      ...
      <div className="flex flex-1 overflow-hidden p-6 gap-6">
          ...
          <div className="card overflow-hidden">
            {SECTIONS.map(({ id, icon: Icon, label, desc }) => (
              <button
                key={id}
                className={cn(
                  'w-full text-left flex items-center gap-3 px-4 py-3.5 transition-colors border-b border-zinc-50 dark:border-zinc-800 last:border-0',
                  section === id ? 'bg-zinc-100 dark:bg-zinc-700 text-zinc-900 dark:text-zinc-50' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-400',
                )}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-zinc-400 dark:text-zinc-500 truncate">{desc}</p>
                </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="card p-6">
            <h2 className="text-base font-bold text-zinc-900 dark:text-zinc-50 mb-1">{current.label}</h2>
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mb-6">{current.desc}</p>
            {renderContent()}
          </div>
        </div>
      </div>
