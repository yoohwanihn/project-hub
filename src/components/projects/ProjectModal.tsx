    ...
    <div className="space-y-2">
      {workflow.map((w) => (
        <div key={w.id} className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-1.5 rounded-lg text-zinc-400 hover:text-red-500 hover:bg-red-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
          >
            <Trash2 size={13} />
          </button>
        </div>
      ...
      {workflow.length < 8 && (
        <button
          type="button"
          onClick={add}
          className="flex items-center gap-2 text-xs text-zinc-900 hover:text-zinc-900 font-medium mt-1"
        >
          <Plus size={13} /> 상태 추가
        </button>
      )}
