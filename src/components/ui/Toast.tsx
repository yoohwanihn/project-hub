...
function ToastItem({ toast }: { toast: Toast }) {
  ...
  return (
    <div
      className={cn(
        'relative flex items-start gap-3 w-80 rounded-2xl border px-4 py-3 shadow-card overflow-hidden',
    ...
    >
      ...
      />
      <Icon size={16} className={cn('mt-0.5 flex-shrink-0', style.text)} />
      <p className={cn('text-xs font-medium flex-1 leading-relaxed', style.text)}>{toast.message}</p>
      <button
